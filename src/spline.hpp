...
/**
  ...
  IV(int i, unsigned r) returns the i-th interpolator associated with vertices.
  IE(int i, unsigned r) returns the i-th interpolator associated with the edge.
...
class Spline {
  public:
    Spline(double h) : _h(h) {
      LegendreGauss quad(2*dim);
      _mass.setZero();
      for (size_t iqn = 0; iqn < quad.npts(); ++iqn) {
        for (size_t i = 0; i < dim; ++i) {
          const double phii = phi(i,_h*quad.tq(iqn));
          _mass(i,i) += _h*quad.wq(iqn)*phii*phii;
          for (size_t j = i+1; j < dim; ++j) {
            _mass(i,j) += _h*quad.wq(iqn)*phii*phi(j,_h*quad.tq(iqn));
          }
        }
      }
      for (size_t i = 0; i < dim; ++i) {
    ...
    static constexpr size_t dim = _d+1;
    static constexpr size_t dofV, dofE;
...
class Spline<3,1> {
  public:
    const Eigen::Matrix<double,3,4> dx{{0,0,-1.},{1.,0,0},{0,0,1.},{0,1.,0}};
    static constexpr int dofV = 2, dofE = 0;
    const &InterpolatorType IV(int i, unsigned r) {
      static int prevR = -1;
      if (prevR < 0) {
        _IV[0].resize(1);
        _IV[0][0].resize(1);
...
class Spline<2,0> {
  public:
    const Eigen::Matrix<double,2,3> dx{{-1.,0},{1.,1.},{0.-1./_h}};
    ...
    const &InterpolatorType IE(int i, unsigned r) {
      static int prevR = -1;
      if (prevR != r) {
  ...
  public:
    ...
    const &InterpolatorType IE(int i, unsigned r) {
      ...
      if (prevR != r) {
        ...
        for (size_t i = 0; i < quad.npts(); ++i) {
          _IE[0][0][i] = {_h*quad.wq(i),_h*quad.tq(i)};
          _IE[1][0][i] = {_h*quad.wq(i)*_h*quad.tq(i),_h*quad.tq(i)};
        }
      }
      prevR = r;
      return _IE.at(i);
